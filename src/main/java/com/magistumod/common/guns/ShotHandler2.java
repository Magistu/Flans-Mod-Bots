...
public class ShotHandler2
{
	
	/**
	 * For any kind of shooting this method should be used. It handles everything including the differentiation between spawning a EntityBullet2 and performing a raytrace
	 * 
	 * @param world             World where the shot is fired
	 * @param shot              FiredShot2 object, created using the guidelines
	 ...
	 */	
	public static void fireGun(World world, FiredShot2 shot, Integer bulletAmount, Vector3f rayTraceOrigin, Vector3f shootingDirection)
	{
	 ...
	 */
	public static void fireGun(World world, FiredShot2 shot, Integer bulletAmount, Vector3f rayTraceOrigin, Vector3f shootingDirection, ShootBulletHandler handler)
	{
		...
		else
		{
			...
			{
				world.spawnEntity(new EntityBullet2(world, shot, rayTraceOrigin.toVec3(), shootingDirection.toVec3()));
				handler.shooting(i < bulletAmount - 1);
			}
		}
	}
	
	private static void createMultipleShots(World world, FiredShot2 shot, Integer bulletAmount, Vector3f rayTraceOrigin, Vector3f shootingDirection, ShootBulletHandler handler)
	{
	...
	private static void createShot(World world, FiredShot2 shot, Float bulletspread, Vector3f rayTraceOrigin, Vector3f shootingDirection)
	{
		...
		Entity ignore = shot.getSourceOptional().isPresent() ? shot.getSourceOptional().get() : shot.getShooterOptional().orElse(null);
	 ...
	 * @param shot              The FiredShot2 instance of the shot
	 ...
	 */
	public static Float OnHit(World world, Vector3f hit, Vector3f shootingDirection, FiredShot2 shot, BulletHit bulletHit, Float penetratingPower)
	{
		...
		if(bulletHit instanceof DriveableHit)
		{
			...
			if (driveableHit.
					...
					ServerHelper.canDamage(shot.getSourceOptional().get(), driveableHit.driveable.getSeat(0).getControllingPassenger(), true)) ///
			{
		...
		else if(bulletHit instanceof PlayerBulletHit)
		{
			...
			Optional<Entity> optionalEntity = shot.getSourceOptional();
		}
		else if(bulletHit instanceof EntityHit)
		{
			EntityHit entityHit = (EntityHit)bulletHit;
			if(ServerHelper.canDamage(shot.getSourceOptional().get(), entityHit.entity, true))
			{
		...
		else if(bulletHit instanceof BlockHit)
		{
			...
			if(bulletType.breaksGlass && mat == Material.GLASS)
			{
				if(TeamsManager.canBreakGlass)
				{
					...
					destroyBlock(worldServer, pos, shot.getSourceOptional().orElse(null), false);
				}
			}
	 ...
	 * @param shot        FiredShot2 instance of the shot
	 * @param detonatePos Location where the detonation should happen
	 */
	public static void onDetonate(World world, FiredShot2 shot, Vector3f detonatePos)
	{
		BulletType bulletType = shot.getBulletType();

...
public class EntityBullet2 extends EntityShootable implements IEntityAdditionalSpawnData
{
	private static final DataParameter<String> BULLET_TYPE = EntityDataManager.createKey(EntityBullet2.class, DataSerializers.STRING);
	
	private static int bulletLife = 600; // Kill bullets after 30 seconds
	public int ticksInAir;
	
	private FiredShot2 shot;
	/**
	 * For homing missiles
	 */
	...
	public EntityBullet2(World world, FiredShot2 shot, Vec3d origin, Vec3d direction)
	{
		this(world);
		ticksInAir = 0;
		this.shot = shot;
	...
	public void onUpdate()
	{
		...
		try
		{
			//This checks if the shooter and/or player can be found. If they are loaded/online they will be included in the FiredShot2 data, if not this data will be deleted/ignored
			if (checkforuuids)
			{
			...
			if (shooter != null)
			{
				shot = new FiredShot2(shot.getFireableGun(), shot.getBulletType(), shooter, player);
			}
			
			checkforuuids = false;
			}
			
			BulletType type = this.getFiredShot2().getBulletType();
			...
			if(!world.isRemote)
			{
				...
				if(!hits.isEmpty())
				{
					for(BulletHit bulletHit : hits)
					{
						...
						currentPenetratingPower = ShotHandler2.OnHit(world, hitPos, motion, shot, bulletHit, currentPenetratingPower);
						if (currentPenetratingPower <= 0f)
						{
							ShotHandler2.onDetonate(world, shot, hitPos);
							setDead();
							break;
						}
					}
	...
	public void readEntityFromNBT(NBTTagCompound tag)
	{
		...
		if (tag.hasKey("fireablegun"))
		{
			...
			fireablegun = new FireableGun(InfoType.getType(gun.getInteger("infotype")), gun.getFloat("damage"), gun.getFloat("vehicledamage"), gun.getFloat("spread"),  EnumSpreadPattern.circle);
		}
		...
		shot = new FiredShot2(fireablegun, type);
	}
	...
	public FiredShot2 getFiredShot2()
	{
		if (shot == null)
		{
			//we dont have this object, therefore we are on the client side and need to construct it
			shot = new FiredShot2(null, BulletType.getBullet(this.dataManager.get(BULLET_TYPE)));
		}
		return shot;
	}
}

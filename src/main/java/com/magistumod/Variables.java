...
public class Variables
{
	...
	{
		public static final String DATA_NAME = "magistumod_mapvars";
		public MapVariables()
		{
			super(DATA_NAME);
		}
		...
		public void syncData(World world)
		{
			...
			if (world.isRemote)
			{
				Main.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			}
			else
			{
				Main.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	{
		public static final String DATA_NAME = "magistumod_worldvars";
		...
		public void syncData(World world)
		{
			...
			if (world.isRemote)
			{
				Main.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			}
			else
			{
				Main.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage>
	{
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world)
		{
			if (context.side == Side.SERVER)
			{
				...
				if (message.type == 0)
					Main.PACKET_HANDLER.sendToAll(message);
				else
					Main.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0)
			{

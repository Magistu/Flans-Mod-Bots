...
public class EntityAIMoveAwayFromEntity<T extends Entity> extends EntityAIBase
{
    ...
    public EntityAIMoveAwayFromEntity(EntityCreature entityIn, Class<T> classToAvoidIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn)
    {
        this(entityIn, classToAvoidIn, Predicates.alwaysTrue(), avoidDistanceIn, farSpeedIn, nearSpeedIn);
    }

    public EntityAIMoveAwayFromEntity(EntityCreature entityIn, Class<T> classToAvoidIn, Predicate <? super T > avoidTargetSelectorIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn)
    {
        this.canBeSeenSelector = new Predicate<Entity>()
        {
            public boolean apply(@Nullable Entity p_apply_1_)
            {
                return p_apply_1_.isEntityAlive() && EntityAIMoveAwayFromEntity.this.entity.getEntitySenses().canSee(p_apply_1_) && !EntityAIMoveAwayFromEntity.this.entity.isOnSameTeam(p_apply_1_);
            }
        };
    ...
    public boolean shouldExecute()
    {
        ...
        {
            this.closestLivingEntity = list.get(1);

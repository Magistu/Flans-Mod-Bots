...
public class ScoreboardWrapper implements IScoreboard
{
    ...
    private boolean test(String datatag) {
        ...
        try {
            Entity entity = CommandBase.getEntity(this.server, (ICommandSender)this.server, datatag);
            NBTTagCompound nbttagcompound = JsonToNBT.getTagFromJson(datatag);
            NBTTagCompound nbttagcompound2 = new NBTTagCompound();
            entity.writeToNBT(nbttagcompound2);
            return NBTUtil.areNBTEquals((NBTBase)nbttagcompound, (NBTBase)nbttagcompound2, true);
        }
        catch (Exception e) {
            return false;
        }

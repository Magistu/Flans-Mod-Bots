    ...
    public void onLivingUpdate() {
        ...
        if (this.world.isRemote && this.getHealth() <= 0.0f) {
            if (!this.exploded) {
                ...
                this.world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + f, this.posY + 2.0 + f2, this.posZ + f3, 0.0, 0.0, 0.0, new int[0]);
            }
        }
        else {
            this.exploded = false;
            float f4 = 0.2f / (MathHelper.sqrt(this.motionX * this.motionX + this.motionZ * this.motionZ) * 10.0f + 1.0f);
            f4 = 0.045f;

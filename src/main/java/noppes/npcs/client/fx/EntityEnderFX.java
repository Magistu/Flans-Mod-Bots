...
public class EntityEnderFX extends ParticlePortal
{
    ...
    public void renderParticle(BufferBuilder renderer, Entity entity, float partialTicks, float par3, float par4, float par5, float par6, float par7) {
        ...
        float f6 = (float)(this.prevPosX + (this.posX - this.prevPosX) * partialTicks - EntityEnderFX.interpPosX + this.startX);
        float f7 = (float)(this.prevPosY + (this.posY - this.prevPosY) * partialTicks - EntityEnderFX.interpPosY + this.startY);
        float f8 = (float)(this.prevPosZ + (this.posZ - this.prevPosZ) * partialTicks - EntityEnderFX.interpPosZ + this.startZ);
        int i = this.getBrightnessForRender(partialTicks);
        int j = i >> 16 & 0xFFFF;
        int k = i & 0xFFFF;
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        renderer.begin(7, DefaultVertexFormats.PARTICLE_POSITION_TEX_COLOR_LMAP);
        renderer.pos((double)(f6 - par3 * f5 - par6 * f5), (double)(f7 - par4 * f5), (double)(f8 - par5 * f5 - par7 * f5)).tex((double)f2, (double)f4).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
        renderer.pos((double)(f6 - par3 * f5 + par6 * f5), (double)(f7 + par4 * f5), (double)(f8 - par5 * f5 + par7 * f5)).tex((double)f2, (double)f3).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
        renderer.pos((double)(f6 + par3 * f5 + par6 * f5), (double)(f7 + par4 * f5), (double)(f8 + par5 * f5 + par7 * f5)).tex((double)f, (double)f3).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
        renderer.pos((double)(f6 + par3 * f5 - par6 * f5), (double)(f7 - par4 * f5), (double)(f8 + par5 * f5 - par7 * f5)).tex((double)f, (double)f4).color(this.particleRed, this.particleGreen, this.particleBlue, 1.0f).lightmap(j, k).endVertex();
        tessellator.draw();
